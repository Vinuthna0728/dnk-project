...
import { Search, ArrowRight, ShieldCheck, CheckCircle2, PackageCheck, HelpCircle } from "lucide-react";
import EscrowModal from "@/components/dwara/ui/EscrowModal";
...
export default function HeroSection() {
  ...
  const handleTrack = (e: React.FormEvent) => {
    e.preventDefault();
    if (trackingNo.trim()) {
      router.push(`/dwara/tracking/${trackingNo.trim()}`);
    }
  };
    ...
    <section className="w-full bg-slate-100 pt-6 pb-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative rounded-xl overflow-hidden bg-white shadow-xl flex flex-col md:flex-row min-h-[420px] border border-slate-200">
          ...
          <div className="w-full md:w-3/5 bg-[#1E3A8A] text-white p-8 sm:p-12 flex flex-col justify-between z-10">
            <div>
            ...
            <div className="flex flex-wrap gap-4">
              <Link
                href="/dwara/verified-artisans"
                className="bg-[#D92D20] text-white px-6 py-3.5 rounded font-bold text-sm hover:bg-red-700 transition flex items-center gap-2 shadow-lg"
              >
              ...
              <button
                ...
                className="bg-white/10 text-white border border-white/20 px-6 py-3.5 rounded font-bold text-sm hover:bg-white/20 transition flex items-center gap-2 cursor-pointer"
              >
          ...
          <div
            ...
            style={{
              backgroundImage: `url('https://images.unsplash.com/photo-1513519245088-0e12902e5a38?auto=format&fit=crop&w=1000&q=80')`,
            }}
          >
        ...
        <div
          id="tracking"
          className="mt-8 bg-white p-6 rounded-xl shadow-md border border-slate-200 grid grid-cols-1 md:grid-cols-3 gap-6 items-center"
        >
          <div className="md:col-span-2 border-b md:border-b-0 md:border-r border-slate-200 pb-6 md:pb-0 md:pr-6">
            ...
            <form onSubmit={handleTrack} className="flex gap-2">
              ...
              <button
                type="submit"
                className="bg-[#1E3A8A] text-white px-6 py-2.5 rounded font-bold text-sm hover:bg-[#2563EB] transition flex items-center gap-2"
              >
      ...
      {/* Interactive Escrow Modal */}
      {isEscrowOpen && (
        <EscrowModal isOpen={isEscrowOpen} onClose={() => setIsEscrowOpen(false)} />
      )}
    </section>
  );
}